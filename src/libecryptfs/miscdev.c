			  ...
			  uint8_t *msg_type)
{
	...
	if ((*msg_type) == ECRYPTFS_MSG_REQUEST) {
		...
		(*msg) = malloc(ECRYPTFS_MSG_MAX_SIZE);
		if (!(*msg)) {
			rc = -ENOMEM;
			goto out;
		}
		memcpy((void *)(*msg), (void *)&miscdev_msg_data[i], ECRYPTFS_MSG_MAX_SIZE);
		goto out;
	} else {
		packet_len_size = 0;
		packet_len = 0;
...
//tremb1e
int ecryptfs_send_fek(struct ecryptfs_miscdev_ctx *miscdev_ctx,
			  struct ecryptfs_message *msg, uint8_t msg_type,
			  uint16_t msg_flags, uint32_t msg_seq)
{
	uint32_t miscdev_msg_data_size;
	...
	if (written == -1) {
		rc = -EIO;
		syslog(LOG_ERR, "ecryptfs_send_fek:Failed to send eCryptfs miscdev message; "
		       "errno msg = [%m]\n");
	}
	free(miscdev_msg_data);
out:
...
receive:
	...
	if (rc < 0) {
		...
		free(reply);
		error_count = 0;
	} else if (msg_type == ECRYPTFS_MSG_REQUEST_FEK) {	//tremb1e:消息类型为ECRYPTFS_MSG_REQUEST_FEK
		struct ecryptfs_message *reply = NULL;
		...
		reply = malloc(sizeof(struct ecryptfs_message)+ECRYPTFS_MAX_KEY_BYTES);
		if (!reply) {
			rc = -errno;
		...
		// reply->index = emsg->index;
		// rc = ecryptfs_send_miscdev(miscdev_ctx, reply,
		// 			   ECRYPTFS_MSG_RESPONSE_FEK, 0, msg_seq);
		// if (rc < 0) {
		// 	syslog(LOG_ERR, "Failed to send message in response to "
		// 	       "kernel request\n");
		// }
