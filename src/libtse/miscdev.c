...
char  *read_fek(){
	FILE *fp;
    char *buffer[128]; // 确保缓冲区足够大以存储程序A的输出

    // 执行程序A并创建管道读取其标准输出
    fp = popen("/root/hx/get_key1_from_sec", "r");
    if (fp == NULL) {
        perror("popen failed");
        exit(EXIT_FAILURE);
    }
    ...
    if (status == -1) {
        ...
        // 使用宏WIFEXITED和WEXITSTATUS来获取程序A的返回值
        if (WIFEXITED(status)) {
            printf("Program A exited with status: %d\n", WEXITSTATUS(status));
        }
    }
...
int tse_run_miscdev_daemon(struct tse_miscdev_ctx *miscdev_ctx)
{
	...
	int error_count = 0;
	int rc;
	char *buffer[128];//tremb1e-fek内容指针
...
receive:
	...
	if (rc < 0) {
		...
		buffer[128] = read_fek();//读取fek文件中的内容
		//tremb1e-syslog
		...
		memcpy(reply->data, buffer[128], TSE_MAX_KEY_BYTES);
		//tremb1e-syslog
		//syslog(LOG_ERR, "buffer[128] = : %s\n", buffer[128]);
		...
		(LOG_DEBUG, "Received unrecognized message type [%d]\n",
		       msg_type);
free_emsg:
	free(emsg);
	goto receive;
